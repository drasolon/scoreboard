...
GameSchema.virtual("url").get(function getUrl() {
  return `/game/${this.id}`;
});

GameSchema.virtual("nonEditableUrl").get(function getNonEditableUrl() {
  return `/game/${this.nonEditableId}`;
});

// Display total scores or total rounds won depending on the game settings
GameSchema.virtual("totalScore").get(function getTotalScore() {
  const totalScore = [];
...
GameSchema.virtual("ranks").get(function getRanks() {
